...
import type { ShiftWithProfile, VacationWithProfile } from '@/lib/types'
...
export default function CalendarView({
  ...
  const fetchData = useCallback(async (y: number, m: number) => {
    ...
    setShifts((shiftsRes.data ?? []) as unknown as ShiftWithProfile[])
    setVacations((vacsRes.data ?? []) as unknown as VacationWithProfile[])
  }, [supabase])
  ...
  shifts.forEach(s => shiftMap.set(s.date, s.profiles.name))
  ...
  vacations.forEach(v => {
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      vacationByDate.get(ds)!.push(v.profiles.name)
    }
  })
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-px rounded-xl border bg-gray-200 overflow-hidden">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <div
              key={dateStr}
              className={`bg-white px-1.5 py-1 min-h-[90px] ${
            ...
            >
              ...
              {assigned && (
                <div className="mt-0.5 rounded bg-blue-100 px-1 py-0.5 text-[11px] text-blue-800 leading-tight truncate">
                  {assigned}
                </div>
              )}
              {onVacation?.map(name => (
                <div key={name} className="mt-0.5 rounded bg-amber-100 px-1 py-0.5 text-[11px] text-amber-800 leading-tight truncate">
                  {name}
                </div>
              ))}
            </div>
      ...
      <div className="mt-3 flex gap-4 text-xs text-gray-500">
        <span className="flex items-center gap-1">
          <span className="inline-block h-3 w-3 rounded bg-blue-100" /> дежурство
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block h-3 w-3 rounded bg-amber-100" /> отпуск
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block h-3 w-3 rounded bg-gray-50 border" /> выходной
        </span>
      </div>
